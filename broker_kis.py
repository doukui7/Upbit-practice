...
class BrokerKIS:
    ...
    def _get_token(self):
        ...
        # KIS는 토큰 발급을 1분에 1회로 제한 (EGW00133)
        if now - self._token_last_attempt < 65:
            if self.access_token:
                return self.access_token
            logging.warning("KIS 토큰 발급 대기 중 (1분 제한)")
            return None

        self._token_last_attempt = now
        ...
        if res.status_code == 200:
            ...
            if "access_token" in data:
                ...
                logging.error(f"KIS Token Response Missing Token: {data}")
                return None
        else:
